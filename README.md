...
By scanning every prerequisite listed in the compiler dependency file (source,
headers, `.inc` files, etc.) for `CONFIG_*` reference strings,
**esp-idf-configdep** ensures that a compile is only marked dirty when one of
its actual configuration dependencies changes. This reduces incremental build
times after small `sdkconfig` edits.
